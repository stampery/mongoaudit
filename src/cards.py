...
class Cards(object):
    ...
    def welcome(self):
        ...
        text = urwid.Text([
            ...
            ('text', ' is a CLI tool for auditing MongoDB servers, detecting poor security settings and performing automated penetration testing.\n\n'),
            ('text italic', "“With great power comes great responsibility”. Unauthorized access to strangers' computer systems is a crime in many countries. Take care.")
        ])
    ...
    def choose_test(self, button=None):
        txt = urwid.Text(
            [
                ('text bold',
                 self.app.name),
                ' provides two distinct test suites covering security in different depth. Please choose which one you want to run:'])

        basic = ImageButton(
            picRead('bars_min.bmp'),
            [('text bold', 'Basic'),
             ('text', 'Analize server perimeter security. (Does not require valid authentication credentials, just the URI)')])
        advanced = ImageButton(
            picRead('bars_max.bmp'),
            [('text bold', 'Advanced'),
             ('text', 'Authenticate to a MongoDB server and analize security from inside. (Requires valid credentials)')])
        content = urwid.Pile([txt, DIV, basic, advanced])

        basic_args = {
            'title': 'Basic',
            'label': 'This test suite will only check if your server implements all the basic perimeter security measures advisable for production databases. For a more thorough analysis, please run the advanced test suite.\n\nPlease enter the URI of your MongoDB server',
        ...
        advanced_args = {
            'title': 'Advanced',
            'label': 'This test suite authenticates to your server using valid credentials and analyzes the security of your deployment from inside.\n\nWe recommend to use the same credentials as you use for your app.\nPlease enter your MongoDB URI in this format:',
    ...
    def uri_prompt(self, title, label, uri_example, tests):
        ...
        form = FormCard(intro, ['URI'], 'Run ' + title.lower() +
                        ' test suite', validate, back=self.choose_test)
    ...
    def display_overview(self, result, title, urn):
        ...
        email_button = LineButton([('text', '📧  Email me the detailed results and how to fix the issues')])
    ...
    def email_prompt(self, result, title, urn):
        ...
        subtitle = urwid.Text([
            ('text', 'The email report contains detailed results of each of the runned tests, as well as links to guides on how to fix the found issues.\n\n'),
            ('text italic', 'This reporting service is runned by Stampery Inc. You will be included into a MongoDB critical security bugs newsletter. We will never SPAM you, we promise!')
        ])
        ...
        card = FormCard(
            ...
            lambda form, email: self.send_email(email.strip(), result, title, urn) if validate_email(
                email) else form.set_error("Invalid email address"),
            lambda _: self.display_overview(result, title, urn))
